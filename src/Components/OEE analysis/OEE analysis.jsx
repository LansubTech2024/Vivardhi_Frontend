...
const OeeDashboard = () => {
  ...
  const filteredMachines = machines.filter((machine) => {
    const matchesZone =
      selectedZone === "all" ||
      machine.zoneName.toLowerCase() === selectedZone.toLowerCase();
    const matchesSearch = machine.zoneName
      .toLowerCase()
  ...
  const zoneOptions = [
    "all",
    ...new Set(machines.map((machine) => machine.zoneName)),
  ];
      ...
      <table>
        ...
        <tbody>
          {filteredMachines.length > 0 ? (
            filteredMachines.map((zone) => (
              <React.Fragment key={zone.zoneName}>
                 {/* Display Machines in Zone */}
                {zone.machines.map((machine) => (
                  <tr key={machine.machineId}>
                    <td className={getColumnClass("machine")}>
                      {machine.machineId}
                    </td>
                    <td className={getColumnClass("oee")}>{machine.avgOEE}</td>
                    <td className={getColumnClass("availability")}>
                      {machine.avgAvailability}
                    </td>
                    <td className={getColumnClass("performance")}>
                      {machine.avgPerformance}
                    </td>
                    <td className={getColumnClass("quality")}>
                      {machine.avgQuality}
                    </td>
                  </tr>
                ))}
            ...
            <tr>
              <td colSpan="5" style={{ textAlign: "center" }}>
                No machines found for the selected zone.
              </td>
            </tr>
          )}
        </tbody>
